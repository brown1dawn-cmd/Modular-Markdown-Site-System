...
export default function Home() {
  ...
  const [selectedTimeline, setSelectedTimeline] = useState<readonly [string, string, string] | null>(null);
  ...
  return <div className="scholar-shell"><div className="scholar-noise" /><header className="scholar-header"><button className="scholar-brand" onClick={() => go("/")}><span className="scholar-brand-mark">王</span><span><strong>WANG XIAOMIN</strong><small>{isZh ? "歷史學者 · 銀河大學" : "HISTORIAN · GALAXY UNIVERSITY"}</small></span></button><nav className={menuOpen ? "scholar-nav is-open" : "scholar-nav"}>{t.nav.map((label, index) => <button key={label} className={t.paths[index].slice(1) === page || (index === 0 && page === "home") ? "is-current" : ""} onClick={() => go(t.paths[index])}>{label}</button>)}</nav><div className="scholar-actions"><button className="scholar-language" onClick={() => setLanguage(isZh ? "en" : "zh")}><Languages size={15} />{isZh ? "EN" : "中文"}</button><button className="scholar-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation">{menuOpen ? <X size={19} /> : <Menu size={19} />}</button></div></header>
    <main>
      ...
      {page === "biography" && <PageShell tag={t.aboutTag} title={t.aboutTitle}><div className="scholar-split"><div><p className="scholar-lede">{t.aboutBody}</p><div className="scholar-quote"><ScrollText size={23} /><p>“{t.quote}”</p><span>{t.quoteBy}</span></div></div><PhotoPlaceholder t={t} /></div><h3 className="subsection-title">{t.timelineTitle}</h3><div className="scholar-timeline">{t.timeline.map((entry) => { const [year, title, detail] = entry; return <button className="timeline-row" key={year} type="button" onClick={() => setSelectedTimeline(entry)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedTimeline(entry); } }} aria-label={`${title} — ${year}`}><span className="timeline-year">{year}</span><span className="timeline-copy"><strong>{title}</strong><small>{detail}</small></span><ChevronRight size={17} /></button>; })}</div></PageShell>}
    ...
    </main>{selectedTimeline && <div className="timeline-dialog-backdrop" role="presentation" onClick={() => setSelectedTimeline(null)}><section className="timeline-dialog" role="dialog" aria-modal="true" aria-labelledby="timeline-dialog-title" onClick={(event) => event.stopPropagation()}><button className="timeline-dialog-close" type="button" onClick={() => setSelectedTimeline(null)} aria-label={isZh ? "關閉" : "Close"}><X size={19} /></button><span className="timeline-dialog-year">{selectedTimeline[0]}</span><h2 id="timeline-dialog-title">{selectedTimeline[1]}</h2><p>{selectedTimeline[2]}</p><span className="timeline-dialog-note">{isZh ? "這裡可以補上更完整的學位、職務、研究成果或相關連結。" : "This space is ready for a fuller account, publication list, or related link."}</span></section></div>}<footer className="scholar-footer"><span className="scholar-footer-mark">王</span><span>{t.footer}</span><span>{isZh ? "雙語學術檔案示範" : "Bilingual academic profile demo"}</span></footer></div>;
}
